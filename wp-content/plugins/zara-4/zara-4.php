...
if( ! class_exists( 'Zara4_WordPressPlugin' ) ) {
	...
	class Zara4_WordPressPlugin {
		...
		function __construct() {
			...
			require_once( $plugin_directory . '/lib/API/ApiUtils.php' );
		...
		private function post_settings_page() {
			...
			// API credentials
			$this->settings->set_api_client_id( $options['api-client-id'] );
			$this->settings->set_api_client_secret( $options['api-client-secret'] );
      ...
      return $meta != null;
		...
		private function optimise_image_from_id( $image_id ) {
			if ( wp_attachment_is_image( $image_id ) ) {
				...
				$this->optimise_thumbnails_from_sizes( $metadata['sizes'] );
